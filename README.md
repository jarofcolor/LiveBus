...
LiveBus为一款轻量的事件总线库，其设计理念为观察订阅模式，与其它事件总线不同的是，订阅和发布直接针对事件本身，而不需要中间调度，所有的事件抽象为`LiveEvent`，`LiveEvent`支持对事件的订阅与发布，支持线程分发，支持生命周期控制下的自动移除。
...
```gradle
implementation 'com.jarofcolor.livebus:livebus:1.1.0'
```
